...
public class FlagWalker {
	 ...
	 */
	public FlagWalker(final int flagBits) {
	 ...
	 */
	public void writeBits(final int numBits, final int bits) {
	...
	public int readBits(final int numBits) {
		...
		int mask = (1 << numBits) - 1;  // create the mask at the least-significant bits
		mask <<= this.nextBitIndex;  // shift the mask to the appropriate index
		final int unshiftedBits = this.flagBits & mask;  // capture the bits we want to read
		final int result = unshiftedBits >> this.nextBitIndex;  // shift the bits to the least significant indices
		this.nextBitIndex += numBits;
		return result;
	}
