...
public class UtilMath {
	 ...
	 */
	public static double distanceFromLine(final DoubleVec2D coord, final Line2D line) {
		// TODO(theimer): doom fast sqrt divide!!! or just cache sqrt(a**2 + b**2) :(

		// the following math is straight from wikipedia
		final double numerator = Math.abs((line.a * coord.x) + (line.b * coord.y) + line.c);
		final double denominator = Math.sqrt(Math.pow(line.a, 2) + Math.pow(line.b, 2));
	 ...
	 */
	public static boolean vecParallelToLine(final DoubleVec2D vec, final Line2D line) {
		assert !vec.isZero() : "vec must have positive magnitude: " + vec;
		final DoubleVec2D lineSlopeVec = new DoubleVec2D(line.b, -line.a);
		final double angle = lineSlopeVec.angle(vec);
	 ...
	 */
	public static int log2Floor(final int val) {
		assert val > 0 : "val must be greater than zero: " + val;
		final int highestBit = Integer.highestOneBit(val);
	 ...
	 * @param val must be greater than zero
	 */
	public static int log2Ceil(final int val) {
		assert val > 0 : "val must be greater than zero: " + val;
		final int highestBit = Integer.highestOneBit(val);
		final int numTrailingZeros = Integer.numberOfTrailingZeros(highestBit);
		if (highestBit == val) {
	 ...
	 * @param val must be greater than zero
	 */
	public static boolean isPow2(final int val) {
		assert val > 0 : "val must be greater than zero: " + val;
	 ...
	 */
	public static DoubleVec2D degreesToVec(final int degrees) {
		...
		assert degrees < UtilMath.CIRCLE_DEGREES : "degrees must be less than CIRCLE_DEGREES: " + degrees;

		final double radians = ((2 * Math.PI) / UtilMath.CIRCLE_DEGREES) * degrees;
		final double x = Math.cos(radians);
		final double y = Math.sin(radians);
	 ...
	 *     from the x-axis.
	 */
	public static double vecToDegrees(final DoubleVec2D vec) {
		assert !vec.isZero(): "vec must have positive magnitude: " + vec;
	 ...
	 */
	public static double euclidianDistance(final DoubleVec2D coordA, final DoubleVec2D coordB) {
	...
	public static boolean doubleEquals(final double d1, final double d2) {
		return Math.abs(d1 - d2) < UtilMath.FLOAT_EPS;
	}
	 ...
	 */
	public static int diffMod(final int valTo, final int valFrom, final int mod) {
		...
		final int directDiff = valTo - valFrom;
		// Note that this will always have the opposite sign of directDiff.
		final int aroundDiff = (mod - Math.abs(directDiff)) * ((directDiff >= 0) ? -1 : 1);
